...
public class GatewayMvcPropertiesBeanDefinitionRegistrar implements ImportBeanDefinitionRegistrar {
    ...
    public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        ...
        AbstractBeanDefinition routerFnProviderBeanDefinition = BeanDefinitionBuilder
                .genericBeanDefinition(RouterFunctionHolder.class)
//                .addConstructorArgReference(getBeanDefinitionName(registry, Environment.class))
//                .addConstructorArgReference(getBeanDefinitionName(registry, GatewayRouteHelper.class))
                .getBeanDefinition();
...
class RouterFunctionHolder
        implements BeanFactoryAware {

    private static final RouterFunction<ServerResponse> NEVER_ROUTE = RouterFunctions.route(new RequestPredicate() {
                                                                                                @Override
    ...
    private final AtomicReference<RouterFunction<ServerResponse>> routerFunctionConfigured = new AtomicReference<>();
    ...
    private RouterFunction<ServerResponse> init() {
        ...
        GatewayRouteHelper gatewayRouteHelper = this.beanFactoryAtomicReference.get().getBean(GatewayRouteHelper.class);
        ...
        properties.getRoutes().forEach(routeProperties -> {
            routerFunctions.put(routeProperties.getId(), gatewayRouteHelper
                    .getRealRouterFunction(routeProperties, routeProperties.getId()));
        });
        properties.getRoutesMap().forEach((routeId, routeProperties) -> {
            ...
            routerFunctions.put(computedRouteId,
                    gatewayRouteHelper
                            .getRealRouterFunction(routeProperties, computedRouteId));
        });
    ...
    public RouterFunction<ServerResponse> getRouterFunction() {
        if (this.routerFunctionConfigured.get() == null)
            this.routerFunctionConfigured.set(init());
